...
#modify SimSUM.csv for fever to be 'high', 'low', 'no' instead of 'none'

# df = pd.read_csv('data/SimSUM.csv', sep=';')
# print(df['fever'].value_counts())
# df.loc[df['fever'].eq('none'), 'fever'] = 'no'
# print(df['fever'].value_counts())
# df.to_csv('data/SimSUM_updated.csv')
# sys.exit()
...
df = pd.read_csv(f'data/CeRTS_SimSUM_1k_test_split_{model_name}.csv')

fever_df = df[~df['LLM_fever'].isin(['high', 'low', 'no'])]
print(len(df))
print(len(fever_df))

#print(fever_df['LLM_fever'].value_counts())
...
#     antibiotics, LLM_antibiotics, antibiotics_conf, pneu, LLM_pneu, pneu_conf = row[['antibiotics', 'LLM_antibiotics', 'antibiotics_conf', 'pneu', 'LLM_pneu', 'pneu_conf']]

#     print(antibiotics, LLM_antibiotics, antibiotics == LLM_antibiotics, antibiotics_conf)
#     x+=1
#     if x == 5:
#         break
...
metrics_df.to_csv(f'data/eval_results/SimSUM_classification_performance_{model_name}.csv')
print(metrics_df.to_string())
print("\nOverall weighted F1 (across features):", round(overall_weighted_f1, 4))

################################
#MEASURE UNCERTAINTY PERFORMANCE
################################
...
calib_df = compute_calibration_and_brier(df, model_name=model_name, outdir="data/SimSUM_calibration_plots", num_bins=10)
print(calib_df.to_string())

calib_df.to_csv(f'data/eval_results/SimSUM_calibration_performance_{model_name}.csv')